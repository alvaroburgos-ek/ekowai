...
-- inflows_count / inflows_unbalanced, outflows_count / outflows_unbalanced, energy_flows_count / energy_unit_mismatch, …; the nine pct_*_agg / pct_econre_agg
-- aggregates are WITHHELD until J-1 — R-1's consumer re-point presupposes their re-emit).
--
...
--
-- =====================================================================================================================
-- iso59020-G-4 · ISO-59020-04 · CR-009 (mandatory_core_indicators_included == true, block) → six per-token clauses over the indicator register (fix round 1)
-- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
-- UPDATE compliance_requirements c SET
--   condition = '(count_rows(indicators, indicator == ''A.2.2_reused_content_inflow'' AND ok == 1) == 1) AND (count_rows(indicators, indicator == ''A.2.3_recycled_content_inflow'' AND ok == 1) == 1) AND (count_rows(indicators, indicator == ''A.2.4_renewable_content_inflow'' AND ok == 1) == 1) AND (count_rows(indicators, indicator == ''A.3.3_reused_from_outflow'' AND ok == 1) == 1) AND (count_rows(indicators, indicator == ''A.3.4_recycled_from_outflow'' AND ok == 1) == 1) AND (count_rows(indicators, indicator == ''A.3.5_biological_recirculation'' AND ok == 1) == 1)',
--   description = 'Plan 3 (iso59020-G-4): jeder der sechs verbindlichen Kernindikatoren von Tabelle 3 (A.2.2 – A.2.4, A.3.3 – A.3.5, L1085–L1111) genau einmal im Indikatorregister ausgewählt oder begründet nicht anwendbar (§7.3.1 L1068–L1071).'
--  WHERE c.id = '3fb354cb-caa3-472b-9a2e-b6064d3ef0e1' AND md5(c.condition) = '119d6e0a38e8ca522d3dc2da70411300';
-- COMMIT;
-- Rollback: RESTORE('3fb354cb-caa3-472b-9a2e-b6064d3ef0e1'). Pinned in-session (equations test, same form as -04-D5): all six once ⇒ 1; five of six ⇒ 0; six + a duplicate A.2.2 ⇒ 0; a duplicate masking a missing token ⇒ 0; an extra Optional row ⇒ still 1; empty register ⇒ 0.
--
-- =====================================================================================================================
...
--
-- =====================================================================================================================
-- iso59020-G-6 · ISO-59020-07 · NEW gate CR-039 (warn): the energy Σ (-07-D2 … D5) are meaningful only while every row is in the common unit (fix round 1, controller ruling)
-- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
-- BEGIN;
-- INSERT INTO compliance_requirements (worksheet_template_id, code, title_de, condition, clause_reference, severity, description)
-- SELECT WS('ISO-59020-07'), 'CR-039', 'Energiesummen nur bei gemeinsamer Einheit sinnvoll (A.4.2)', '(IF energy_unit_mismatch > 0 THEN energy_unit_mismatch == 0)', 'A.4.2', 'warn',
--   'Plan 3 (iso59020-G-6): Σ EIRENE / EORENE / EITE / EOTE (ISO-59020-07-D2 … D5) sind nur sinnvoll, solange jede Zeile des Energieregisters in energy_unit_common erfasst ist — „A common suitable measurement unit (e.g. MJ, kWh) shall be selected for the quantification of all relevant energies“ (L2271–L2272).'
-- WHERE NOT EXISTS (SELECT 1 FROM compliance_requirements x WHERE x.worksheet_template_id = WS('ISO-59020-07') AND x.code = 'CR-039');
-- COMMIT;
-- Rollback: DELETE FROM compliance_requirements WHERE worksheet_template_id = WS('ISO-59020-07') AND code = 'CR-039' AND description LIKE 'Plan 3 (iso59020-G-6)%'.
--
-- =====================================================================================================================
...
-- -- the seven CONSUMER_ADD statements above presuppose the J-1 re-emit of the seven aggregates (withheld since fix round 1).
...
--
-- =====================================================================================================================
-- iso59020-D-10 · ISO-59020-05 · `pct_linear_inflow` ↔ `inflows.pct_linear (per row) / pct_linear_agg (withheld, J-1)`
-- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
--
-- =====================================================================================================================
-- iso59020-D-17 · ISO-59020-06 · `pct_linear_outflow` ↔ `outflows.pct_linear (per row) / pct_linear_out_agg (withheld, J-1)`
-- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
--
-- =====================================================================================================================
-- iso59020-J-1 · ISO-59020-05 / -06 / -07 · the cross-X aggregation rule — nine Σ/Σ aggregates WITHHELD (fix round 1, controller ruling)
-- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
-- -- on RATIFIED (Σ/Σ): add the nine rows to equations/iso59020.ts + the nine derived fields to field-configs/iso59020.ts (footers), re-emit 20260917102110 / 20260917102120 — the forms above verbatim. On RATIFIED (mean): the mean_rows forms instead.
-- Rollback: —
--
-- =====================================================================================================================
-- iso59020-J-2 · ISO-59020-04 · mandatory_core_indicators_included_code — per-token "exactly once" form over the six Mandatory rows of Table 3 (fix round 1)
-- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
-- -- alternative: UPDATE equations SET formula = … '>= 1' per clause WHERE equation_number = 'ISO-59020-04-D5' AND description LIKE 'Plan 3:%'.
-- Rollback: —
--
-- =====================================================================================================================
...
--
-- =====================================================================================================================
-- iso59020-J-9 · ISO-59020-07 · Formula (A.8) block prints "⋅1000" while its legend reads "in %" — ·100 encoded (reclassified from U-2 to a judgment, fix round 1)
-- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
-- Evidence: "The calculation of the circularity indicator is performed by applying Formula (A.8): (  E IRENE( X ) − EORENE( X ) PECONRE( X ) =  )  ⋅1000 (A.8)  E ITE( X ) − EOTE( X )  where PECONRE( X ) is the average energy (X) consumed that is renewable energy, in %; E IRENE( X ) is the renewable energy (X) inflow, in MJ (or in kWh); EORENE( X ) is the renewable energy (X) outflow, in MJ (or in kWh); E ITE( X ) is the total energy (X) inflow, in MJ (or in kWh); EOTE( X ) is the total energy (X) outflow, in MJ (or in kWh)." (L2275–L2292); "PECONRE( X ) is the average energy (X) consumed that is renewable energy, in %;" (L2284); "%REUI( X ) =   ⋅100 (A.1)" (L1929); ")  ⋅1000 (A.8)" (L2280)
-- -- if the PDF confirms ⋅1000: UPDATE fields SET ui_config = jsonb_set(…pct_econre expr… '* 1000') on ISO-59020-07 energy_flows; the J-1 re-emit with * 1000; and a finding on prod A.8.
-- Rollback: —
--
-- =====================================================================================================================
...
--
-- =====================================================================================================================
-- iso59020-F-2 · ISO-59020-04 · duplicate indicator rows (the language has no distinct-count over a register) — CLOSED for the six Mandatory rows by the per-token form (fix round 1)
-- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
--
-- =====================================================================================================================
-- iso59020-X-3 · ISO-59020-07 · prod hygiene observation — the A.8 formula LHS is PECONRE(X) while output_symbol is pct_ECONRE_X (not this task's write)
-- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
-- Evidence: " E IRENE( X ) − EORENE( X ) PECONRE( X ) =  )  ⋅1000 (A.8)  E ITE( X ) − EOTE( X ) " (L2278–L2281); "PECONRE( X ) is the average energy (X) consumed that is renewable energy, in %;" (L2284)
-- -- none (observation).
-- Rollback: —
--
