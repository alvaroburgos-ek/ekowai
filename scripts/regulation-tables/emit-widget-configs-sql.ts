...
/**
 * The ONE write boundary of every generated `.sql` file (fix round 1, reviewer minor 4).
 *
 * `.gitattributes` stores these files with LF, but on Windows the TS modules the emitters
 * READ are checked out with CRLF, so a multi-line `verbatim_quote` / `verification_quote`
 * carried `\r\n` into the emitted SQL: the working tree drifted from the committed blob
 * (which git had normalised to LF on the way in), and a migration applied straight from
 * such a worktree would write `\r\n` into a quote where a fresh checkout writes `\n`.
 * Normalise CRLF→LF here, once, for every emitter.
 *
 * A LONE `\r` is CONTENT — the committed blobs carry a few inside quotes — and is left
 * exactly as it is: stripping it would alter a verbatim quote.
 */
export function writeSql(path: string, sql: string): void {
...
if (process.argv[1]?.endsWith('emit-widget-configs-sql.ts')) {
  ...
  for (const [file, sql] of emitWidgetConfigSql(WIDGET_MIGRATION_ENTRIES)) {
    writeSql(`scripts/migrations/${file}.sql`, sql);
    written++;
  }
  writeSql('scripts/rollback-20260916130000-widget-configs.sql', emitWidgetRollbackSql(WIDGET_MIGRATION_ENTRIES));
