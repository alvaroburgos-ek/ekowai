...
const SCHEMA_MIGRATION = '20260911100000_guideline_to_tool_schema.sql';
const SIGN_OFF_DOC = 'docs/superpowers/specs/2026-09-11-guideline-to-tool/SIGN-OFF-plan-2b.md';
const JOIN = 'FROM worksheet_templates w JOIN standards s ON s.id = w.standard_id';
...
export const WIDGET_MIGRATION_ENTRIES: readonly WidgetEntry[] = [
  ...
  },
  {
    ...
    retires: 'REGISTER_CONFIGS_FALLBACK.pollutant_register + REGISTER_FLAG_KEYS (src/lib/eval/register-configs.ts)',
  },
  {
...
const comment = (line: string) => (line === '' ? '--' : `-- ${line}`);
...
export function emitWidgetConfigSql(entries: readonly WidgetEntry[]): Map<string, string> {
  ...
  for (const e of entries) {
    ...
    const lines = [
      ...
      ...(e.gated ? [`-- GATED: do not apply before sign-off ${e.gated} is RATIFIED (${SIGN_OFF_DOC}).`] : []),
      ...(e.gated && e.gated_note != null ? e.gated_note.split('\n').map(comment) : []),
      'BEGIN;',
