...
# Fix round 1 (review verdict on `8e693e0`: APPROVED with Important items)

- **Commit:** see the reply (one commit, Fable trailer; `git checkout -- scripts/reasoning-map/` before staging; tree clean after).
- **Nothing applied to prod.** No DB write; the three Plan-3 migrations were regenerated by the Task 0 emitters only.

## REQUIRED

1. **TAB14 `boeschung_max` → `boeschung_m_min`** (`regulation-tables-seed-a138.ts`): the printed "1 : 1,5 oder flacher" / "≤ 1 : 1,5" (L2259, unit "1: m") is a MINIMUM of the slope ratio m — flatter = larger m. Column renamed, unit `m (1:m)`, value 1.5 and the quote unchanged; one comment paragraph in the Tab. 14 builder explains the sense ("a slope 1:2 satisfies it, 1:1 does not"). Test `regulation-tables-seed-a138.test.ts` updated (TAB14 row expectations). Seed migration + rollback regenerated (`20260917100100_regulation_tables_seed_a138_p3.sql`: 8 occurrences of `boeschung_m_min`, 0 of `boeschung_max`); freshness pins green.
2. **`feasibility_code` label + description** (`field-configs/a138.ts`): label now "Umsetzbarkeit nach Tabelle 3 als Code (1 = alle Spalte-2-Kriterien erfüllt · 2 = nicht nachgewiesen möglich · 3 = ein Spalte-4-Kriterium trifft zu)"; the description names the two column-4 fold-ins into code 2 — Trinkwasserschutzgebiet with non-negligible risk (L747, no risk field ⇒ any zone is 2) and k_f < 1·10⁻⁶ m/s without possible Anschluss/Ableitung (L748, no field). Field-config migration regenerated.
3. **a138-C-5** now states the coupling explicitly: ratifying D-2b-3 option (a) MUST re-key to `a138_tier` (never a new `tab6_tier`) and cannot be ratified independently of C-5/E-3; `SIGN-OFF-plan-2b.md` D-2b-3 carries the reverse one-line cross-reference.
4. **a138-E-3**: sentence added recording the controller's acceptance of the staged placement (fill-mode lookup_fill with a missing `bbz_band` key would hide the engineer's input; prod effect identical to 2b's gated binding); same line in the STAGED SQL block.

## MINOR

- (a) **a138-P-1** proposes `kann` (L791 "kann in begründeten Fällen abgewichen werden" = the spec's `kann` token); the block notes that `kann` gives a select over the printed `bk` alternatives without a reason textarea, `anhaltswert` being the alternative if the "begründeten Fällen" reason must be captured.
- (b) **Playbook**: "Composed `override_quote`" note added before "Token budget note" (two verbatim, line-cited fragments joined with " — " are acceptable; `verify-regulation-tables.ts` does not cover `override_quote`).
- (c) **`n_R_MRS`** left as `facility_type_selected IN {'MRE', 'MRS'}`: narrowing to `== 'MRS'` WOULD change which rows are hidden (the field would additionally disappear when MRE is selected on the MRS worksheet), so per the instruction it stays; the MRE case is noted under a138-C-2 for the owner.
- (d) **`tab7AsTable` comment** explains the `tier1_none` token: the TAB5 tier token for VW1/V1/BG1 — "none" refers to Tab. 6 (no A_C/A_S,m requirement), not Tab. 7, which prints 40 % / 50 % for that group (L985); kept for key-string equality with TAB5 (G-A3).

## Raw output

```
pnpm vitest run --project unit <seed-a138, field-configs-a138, equations-a138, generated-sql-freshness, emit-seed-sql>
 Test Files  5 passed (5) · Tests  38 passed (38)
pnpm -s tsx scripts/regulation-tables/verify-regulation-tables.ts a138_p3 "C:\Users\Ekowai\Desktop\Guidelines\DWA-A-138-1\DWA-A_138-1_WD (5).md"
 81/81 quotes verbatim in C:\Users\Ekowai\Desktop\Guidelines\DWA-A-138-1\DWA-A_138-1_WD (5).md
pnpm test
 Test Files  244 passed | 1 skipped (245)
      Tests  2374 passed | 1 expected fail | 1 skipped (2376)
   Start at  16:25:03
