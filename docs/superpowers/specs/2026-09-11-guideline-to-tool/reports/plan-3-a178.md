...
# Fix round 1 (review verdict on `55debd6`: APPROVED with minors; on top of `00ad41a`)

- **Commit:** see the reply (one commit, Fable trailer; `git checkout -- scripts/reasoning-map/` before staging; tree clean after). **Nothing applied to prod** — no new prod read was needed; the seed and field-config migrations were regenerated by the Task 0 emitters only (the equations file is byte-identical).

1. **STAGED G-7 label:** the REQ-05a block's description and rollback selector read `Plan 3 (a178-G-6)` (a slip of the G-renumbering) → `Plan 3 (a178-G-7)`; the STAGED file now carries each of G-1 … G-7 under its own id.
2. **`n_rbf_min` unit as PRINTED:** L979 prints "≥ 10 a" (the §3.2 symbol table L354 prints n_RBF in 1/a) — the row's `unit` cell is now `'a'` verbatim, the label names the §3.2 unit, new sign-off **a178-U-5** (PDF read + a one-cell `jsonb_set` if the print is "1/a"); seed re-emitted (`20260917101400`), verifier **51/51**, seed test pins the unit.
3. **S6_LIMITS descriptive modals:** new **a178-O-5** names `h_rr_min` / `h_rr_max` ("liegen zwischen"), `deckschicht_cm` ("besteht aus"), `langzeitsimulation_min_a` ("basiert auf … mindestens") under the table's `locked` policy — no widget binds S6_LIMITS today, no policy change.
4. **Teilflächen note** states that only complete rows enter the sums — a Mischsystem row without `e_0_i` drops out of `B_RBF_zu_calc` (and `A_E_b_a_calc`) until e_0 is entered; field-config migration re-emitted (`20260917101410`).
5. **a178-O-4** is marked a **[CODE] tooling candidate** ("per-row alternatives on RegulationRow", queued by the controller for the final wave) instead of a DEFER.
6. **STAGED count sentence** (§7 of this report): 23 ☐ blocks, one of them the combined D-4 … D-11 pairs.

Raw output (once, before the commit): sheet fragments machine-checked **63/63**; `pnpm vitest run --project unit <seed-a178, field-configs-a178, equations-a178, register-a178-teilflaechen, generated-sql-freshness>` → 5 files / 35 passed (+ the unit pin: seed-a178 9/9); `pnpm test` → `Test Files 302 passed | 1 skipped (303) · Tests 2806 passed | 1 expected fail | 1 skipped (2808)` (2803 + 3 from the 12c round-3 commit, net 0 from this round beyond the one pin); `pnpm -s typecheck` → exit 0; `pnpm -s eslint <3 touched .ts files>` → exit 0. **The integration harness was not re-run** (per the controller): this round changed one table cell, one register note, sheet / STAGED / report text — no engine, harness seed, or prod-shaped row the A-178 harness reads; the 27/27 run above stands. Sign-off blocks: **57** (+O-5, +U-5).
