...
# Fix round 2 (re-review of `2bc4035..755d952`: PASS — closing round, one commit)

Guard hardening (`scripts/regulation-tables/emit-field-configs-sql.ts`): (a) equation `input_symbols`, outputs and the hidden symbol are matched through `normalizeSymbol` (`src/lib/eval/normalize-formula.ts`, the same rule `formula.ts` applies), so `r_D(n)` ↔ `r_D_n` cannot slip past — pinned both ways plus a function-like output; (b) the chain walk now runs for `create` entries too (`producerChain(…, { skipDirect: true })` — a created field has no consumers of its own but may complete a dangling `input_symbols` reference of a consumed equation; pinned with a created `B_CSB` feeding `A262-24-D1 → B_CSB_KomKA (consumed by A262-21)`); (c) a chain whose only consumer is the owner worksheet still refuses and now says `consumed only by itself (<ws>) — prod data oddity` (pinned; the mixed case keeps the plain list). Three new cases in `emit-field-configs-sql-transitive.test.ts` (40/40 across the four emitter suites); `field-configs-din1989-1.test.ts` re-pinned to the self-consumer wording (`E_R` / `BW_a` list `DIN-1989-1-04` itself — observation added under Task 3 on the sign-off sheet). Re-emit of the three field-config migrations (`a138 20260917100110`, `din1989_1 20260917100210`, `a262e 20260917100310`): **byte-identical** (`git status` shows no migration/rollback change). Editorial: the dangling sentence in `a262e-STAGED-plan3-rulings.sql` (P-2 block) replaced by "The printed relaxation, read in-session:"; the a262e-C-1 evidence line now reads "… whose output has non-empty `consumer_worksheets` (emitter guard …)". Raw: `pnpm test` 254 files / **2460 passed** (+3) · typecheck exit 0 · eslint on the three touched files exit 0.
