...
- Candidate refinement (NOT implemented, owner's call): let the IF-guard exemption treat a bare literal as equal to a quoted one when the captured prior holds no field of that name on the worksheet — would clear the 5 **Q** rows without a gate edit.
- Pre-existing test assertions changed (prior-shape pins only): `build-prior-snapshot.test.mjs` `_meta` toEqual gains `gate_rows: 0`; the freshness pins of `field-configs-{a138,din1989-1,a262e,m277e,m820_3,din276}.test.ts` now emit with `gate_guard: 'warn'` + pin the exact refusal count + pin that the default mode throws; `field-configs-din276.test.ts` "the emitted rule is accepted" (GK_mwst_satz_pct) now pins the REQ-26 empty-condition refusal instead. `build-prior-snapshot.mjs` lost its `#!` shebang line (vitest 4.1.5 could not parse the module in isolation with it; usage is `node scripts/…` and unchanged).
- Evidence: `scripts/regulation-tables/emit-field-configs-sql.ts` (`guardExempts`, `gateReaders`, `priorSnapshotWarnings`, `PRIOR_SQL.gates`), `scripts/regulation-tables/build-prior-snapshot.mjs`, `src/lib/eval/field-configs/types.ts` (`PriorGateRow`), the 13 re-captured `*.prior.json`; pins `scripts/__tests__/emit-field-configs-sql-gates.test.ts` (RED 10/10 against the old emitter, then GREEN), `scripts/regulation-tables/__tests__/build-prior-snapshot.test.mjs`; playbook §Step 4 "The guard is GATE-AWARE (Task 12c)"; task report `.superpowers/sdd/2026-09-16-guideline-to-tool-plan-3-encode-29-standards/task-12c-report.md`.
- ☐ RATIFIED ☐ REJECTED ☐ DEFER
