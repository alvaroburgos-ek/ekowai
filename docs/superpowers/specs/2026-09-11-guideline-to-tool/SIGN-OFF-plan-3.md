...
### iso59020-G-4 · ISO-59020 · ISO-59020-04 · CR-009 (mandatory_core_indicators_included == true, block) → the register-derived code
- Class: gate-guard
...
- Proposed SQL / config: STAGED block G-4 (6 statements; rollback: RESTORE('3fb354cb-caa3-472b-9a2e-b6064d3ef0e1'). Pinned in-session: 5 of 6 covered ⇒ code 0; all six ⇒ 1; empty register ⇒ 0.)
- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
### iso59020-D-10 · ISO-59020 · ISO-59020-05 · `pct_linear_inflow` ↔ `inflows.pct_linear (per row) / pct_linear_agg (-05-D10)`
- Class: deactivation (amendment-K pair)
...
### iso59020-D-17 · ISO-59020 · ISO-59020-06 · `pct_linear_outflow` ↔ `outflows.pct_linear (per row) / pct_linear_out_agg (-06-D10)`
- Class: deactivation (amendment-K pair)
...
### iso59020-J-1 · ISO-59020 · ISO-59020-05 / -06 / -07 · the cross-X aggregation rule (mass-weighted Σ share / Σ total — pct_*_agg, pct_econre_agg)
- Class: text-only-formula
...
- Proposed SQL / config: alternative: replace the seven pct_*_agg formulas by mean_rows(<reg>, pct_<x>) (unweighted) — UPDATE equations SET formula = … WHERE equation_number = … AND description LIKE ''Plan 3:%''.
- ☐ RATIFIED ☐ REJECTED ☐ DEFER

### iso59020-J-2 · ISO-59020 · ISO-59020-04 · mandatory_core_indicators_included_code requires count == 6 (the number of "Mandatory" rows printed in Table 3)
- Class: text-only-formula
...
- Evidence (verbatim, transcript line): "The resource inflows and resource outflows of the system in focus shall be quantified and fully balanced with the use of the mandatory indicators in Clauses A.2 and A.3, taking changes in stocks into account. If a core circularity indicator is not applicable, the organization should explain why and can count the indicator value as zero." (L1068–L1071); "Table 3 — Core circularity indicators" (L1081)
- Proposed SQL / config: alternative: UPDATE equations SET formula = ''mandatory_core_indicators_included_code = if(count_rows(indicators, ok == 0) == 0, 1, 0)'' WHERE equation_number = ''ISO-59020-04-D5'' AND description LIKE ''Plan 3:%''.
- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
### iso59020-U-2 · ISO-59020 · ISO-59020-07 · Formula (A.8) block prints "⋅1000" while its legend reads "in %"
- Class: unreadable-cell
...
- Evidence (verbatim, transcript line): "The calculation of the circularity indicator is performed by applying Formula (A.8): (  E IRENE( X ) − EORENE( X ) PECONRE( X ) =  )  ⋅1000 (A.8)  E ITE( X ) − EOTE( X )  where PECONRE( X ) is the average energy (X) consumed that is renewable energy, in %; E IRENE( X ) is the renewable energy (X) inflow, in MJ (or in kWh); EORENE( X ) is the renewable energy (X) outflow, in MJ (or in kWh); E ITE( X ) is the total energy (X) inflow, in MJ (or in kWh); EOTE( X ) is the total energy (X) outflow, in MJ (or in kWh)." (L2275–L2292)
- Proposed SQL / config: if the PDF confirms ⋅1000: UPDATE equations SET formula = … * 1000 WHERE equation_number = ''ISO-59020-07-D6'' …; and the register column expr in ui_config.
- ☐ RATIFIED ☐ REJECTED ☐ DEFER
...
### iso59020-F-2 · ISO-59020 · ISO-59020-04 · duplicate indicator rows are not detected (the language has no distinct-count over a register)
- Class: interface-gap
...
- **The 25 amendment-K pairs** are the price of the per-flow / per-indicator / per-source registers over a prod model that asks every fact once (one X per worksheet); 0 stored values / 0 instances make every retirement data-free today.
