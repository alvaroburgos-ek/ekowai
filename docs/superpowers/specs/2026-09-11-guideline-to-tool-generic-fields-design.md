...
| Repeatable "one of N" groups | ~230 | 3 bespoke editors on main (surface inventory, pollutant register, rainfall tables) + 36 config entries (deduped from the 38 uncommitted in `_wt-fll`) |
...
- **Phase 2 — the 36 configs.** Script emits one migration per standard (`widget`, `ui_config`, options into `enum_values`). Pin: `fromDbField(row)` deep-equals `SELECTION_CONFIGS[symbol]` for all 36, then the TS registry is deleted. Explicit conflict: `a138_anlagentyp_kandidaten` prod `enum_values` vs config labels → keep prod.
