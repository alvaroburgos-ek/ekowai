...
**Reference implementations to copy the shape from:** single-source — `feat/a138-07-surface-singlesource` (merged `805686d`); accessor — `feat/kostra-rdn-accessor`; multi-table — `feat/rainfall-multi-table` (Piece 2, live 2026-06-29); governing engine — `feat/governing-duration-engine` (Piece 1); 2D grid + per-facility T_n + flood-as-governing-profile — `feat/rainfall-2d-grid` (live 2026-06-29); upstream-value-derived-from-downstream-iteration (A138-10 Q_zu ← basin governing D; acyclic when the upstream input is iteration-independent) — `feat/a138-10-auto-qzu`; write-lock/materialization — `origin/main` `294c89d`.

### 10c. Watch-for — cross-guideline symbol leaks in server-side value lookups

When materializing/deriving a value **server-side** (e.g. `materialize-*` in `saveWorksheet`, or any scalar gather that resolves inputs *by symbol*), **scope the symbol lookup to the current standard's worksheet set — never "all active fields in the project."** A project can carry multiple standards; a project-wide first-wins symbol resolver will silently grab a *different guideline's* field if a future DIN/ISO/FLL/EN reuses a symbol name (e.g. `A_C`, `D`, `Q_S`). That is a latent violation of the strict-separation invariant — invisible only while symbols happen to be unique. **Audit rule:** every by-symbol resolver must be standard/worksheet-scoped. (Known instance to fix as a cleanup pass: `materializeBasinGoverning`'s scalar lookup — `feat/a138-10-auto-qzu` — safe today only because 138's hydraulic symbols are unique.)

### 10d. Caveat — same-symbol seeding supersedes free-typing only on CLEAN projects

When a consolidation moves a datum from free-typed (consumer-owned) to derived (producer-owned) and the consumer keeps a same-symbol field (deactivated), the producer's value reaches the consumer via the **same-symbol seeding path** (`loadSameSymbolValues` → `initialValues` step-2). That path is **step-2**: a project's OWN already-persisted local param (step-1) **wins**. So the consolidation **auto-supersedes free-typing only on CLEAN projects** (no local value yet) — a project that already TYPED the value keeps **showing it (shadowed)** until that `project_parameters` row is cleared. If true supersession across existing projects is required, the migration must also DELETE the stale consumer param rows (a data change — usually not worth it; confirm per project). Observed: `feat/a138-10-auto-qzu` (A138-10 `r_D_n`/`D_min`) — clean PLT-HS-01 derives, throwaway Köln-Lindenthal's typed 200/15 shadows (left as-is).

### 10e. Audit checklist: recurring gap CLASSES (defect-hunt — apply to EVERY guideline)

§10b asks "which *patterns* to apply." This asks "which *defects* to hunt." These ten gap classes recurred across the DWA-A 138-1 build; each is **standard-agnostic**. When you audit any future guideline (VSME, DWA-A 117, DIN/ISO/EN/FLL/VDI/ASTM), walk every item: **what to check · the fix shape · the audit trigger**. The 138 instances are worked examples — the point is to **APPLY the proven fix, not re-derive it**.

> **Overarching principle (governs all ten).**
> **(1) Ground in the actual SOURCE document** (PDF/MD in `Desktop/Guidelines/`), never UI dumps, agent reports, or the encoded DB alone — reading the source caught wrong implementations repeatedly on 138 (e.g. `A_S ≡ A_S,m` in the Einfaches Verfahren; `r_D(n)` is iteration-governed not a free key; Tab.6 loading is a treatment-side check, not basin sizing). **(2) A value appearing ≠ a value being correct** — verify each displayed/persisted number against its formula AND the source. This caught the legacy-sum artifacts `Q_zu=45` and `q_S_AC=4836` (both were a naive sum of unrelated inputs that *looked* plausible).

| # | Gap class | What to check | Fix shape | Audit trigger | 138 worked example |
|---|---|---|---|---|---|
| 1 | **Free-typed value that should derive** | Every "Results/Derived"-looking field has a producing equation/lookup and is **not** hand-editable | Derive from a single producer; make read-only; register the equation | Scan for derived-looking fields that are free inputs | `A138-10` r_D/D, `A138-11` f_K, A_S/A_S,m |
| 2 | **Equation not on the engine whitelist → legacy sum-evaluator** | Every equation with a **non-additive** formula (×, ÷, caps, `min`/`max`, powers) is in **BOTH** whitelists (`engine-whitelist.ts` + `whitelist.ts`) | Add to both lists + a regression-guard test | Cross-reference ALL equations vs whitelist; flag any non-sum formula missing. **Fixture-local whitelists mask this — tests pass while prod mis-computes** | `A138-10:3`, `A138-11:5/6`, `A138-13:9` (the `Q_zu=45`/`q_S_AC=4836` artifacts) |
| 3 | **Stored formula/`input_symbols` ≠ real field symbols (engine is CASE-SENSITIVE)** | Each equation's `formula` AND `input_symbols` resolve to actual field symbols, **exact case**; no doubled/malformed text | Equations-text migration correcting both, at source | Validate symbol resolution per equation (the engine only normalizes `fn(x)`→`fn_x`, never case) | `f_Ort`→`f_ort`, `f_Methode`→`f_methode`, `k`→`k_f` (`20260629160000`) |
| 4 | **Consumer not registered → "Fehlt: X" though X exists upstream** | Every cross-worksheet input has the consumer listed in the producer's `consumer_worksheets` | Register the consumer edge | Trace each "expected from upstream" input to a registered producer→consumer edge; a wiring gap masquerades as missing data | `A138-13` ← `A_VA`/`Q_Dr` prereq (`20260629150000`) |
| 5 | **Stale param row (incl. `value=NULL`) shadows the inherited/derived value** | After ANY consolidation, **zero** stale consumer param rows remain | **DELETE** the stale rows — don't just deactivate the field (deactivating leaves the data row, which still wins step-1 > step-2; see §10d) | On every consolidation, sweep the consumer's `project_parameters` rows for the moved symbol | A138-10 `A_C` NULL shadow (one-off DELETE) |
| 6 | **Display-only output not persisted → downstream can't inherit** | Every derived value that another worksheet consumes actually persists to `project_parameters` (`source_type='derived'`) on save — not just displayed | Add a materialize step | For each cross-worksheet derived value, verify DB persistence, not just on-screen display. **Note:** ordinary whitelisted *equation* outputs persist via the standing `4a49607` fix; only **non-equation/`derivedExtras`** outputs need an explicit materialize | basin Task-2b (`materializeBasinGoverning`); the inverse: `r_D_n`/`D_min` are non-equation outputs → persisted `entered` not `derived` (provenance bug, Piece-1 close-out) |
| 7 | **Compliance gate that doesn't actually enforce (grammar limits)** | Every `severity='block'` requirement's `condition` is expressible/enforced by `evaluate.ts` (no `==`, no chained AND/OR, no unsupported functions) | Gate-grammar work (rewrite to supported grammar / extend evaluator) | Validate each compliance condition against the gate grammar. **A green gate ≠ an enforcing gate** — the truth is the computed value, not the gate color | `REQ-15` (`q_S_AC≥2 AND (…OR f_Z==1.2)`); `A138-16:13` gated nowhere |
| 8 | **Iteration/search hits a TABLE BOUNDARY instead of a true maximum** | A governing-value iteration finds an **interior "eindeutiges Maximum,"** not the last tabulated row; non-turnover is flagged | Boundary-detection flag + adequate data range | For any "find the governing X by iteration," verify it detects no-turnover rather than silently returning the edge — and that the data range is wide enough (cross-check the source's prescribed range) | governing-D returned `D=1440` (table max), V_VA monotonic — the signature of `k_i` too low (q_S_AC≪2 fails the Anwendungsbedingung); §5.3.3.7 wants a max per DWA-A 117 (Piece-1 close-out) |
| 9 | **Symbol overloaded across facility types / sub-cases** | Before consolidating a symbol, enumerate **ALL** its consumers and confirm the single-source rule holds for **each** | Per-context scoping, **never** a blanket auto-equate | On any "consolidate symbol X," list every consumer and verify per-context; one blanket rule corrupts the others | `A_S` ≡ `A_S,m` for ponding (Mulde/Becken) BUT computed by its own equation for Flächenversickerung (`A138-16:12`) / Schacht (`A138-21:34`) |
| 10 | **Server-side by-symbol lookup scoped to ALL project fields → cross-guideline leak** | Every server-side by-symbol resolver is scoped to the current **standard/worksheet set**, never "all active fields in the project" | Scope the lookup to the standard | Any server-side symbol gather must be standard-scoped — a first-wins resolver grabs another guideline's field on symbol collision (`A_C`, `D`, `Q_S`…). Full treatment in **§10c** | `materializeBasinGoverning` scalar lookup (latent; safe only while 138 symbols are unique) |

**How to run it:** for each guideline, walk the ten rows and produce a defect list (each: class # · source citation or DB evidence · the worksheet/field/equation · the fix shape from the table). That list feeds the §10b per-guideline matrix and then §4 (3-plan) + §5 (deploy runbook). Classes 1–6 are usually *build* fixes; 7–8 pair with the **compliance-gating + governing-duration close-out** work; 9–10 are *scoping discipline* that must be settled BEFORE any consolidation migration is written.
