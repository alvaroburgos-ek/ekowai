...
- **Missing-column safety — CONDITIONAL ON TABLE TYPE.** `legacyDesignColumn` table (existing 1D data, no real columns) → serve the single curve for ANY T_n (`legacy`), **never withhold** (withholding would break every existing project). Native 2D grid → requested column populated → `ok`; that specific column empty/absent (or T_n null) → **`missing` ⇒ WITHHOLD with a cause** (surface-source "fehlend" style), never a neighbouring column. No `designReturnPeriod` param.
...
- [ ] **Step 1a: `resolveColumn` tagged, conditional on table type — failing test first.** `rainfall-2d-resolve.test.ts`: legacy table → `{status:'legacy'}` for ANY T_n (and even `T_n=null`); native grid, column populated → `{status:'ok'}`; native grid, requested column empty/absent (or `T_n=null`) → `{status:'missing'}`. Signature: `resolveColumn(table, T_n: number | null)` — NO `designReturnPeriod` param.
- [ ] **Step 1b: Implement.** Branch on `table.legacyDesignColumn`: true → serve `__legacyValue` for any T_n (`legacy`); false (native) → exact populated column → `ok`, else `missing`. Run resolve + `rainfall-2d.test.ts` → PASS. Commit `refactor(eval): resolveColumn tagged status, conditional on table type (legacy serves any T_n; native withholds missing column)`.
