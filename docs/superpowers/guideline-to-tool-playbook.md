...
**Encoding traps (Plan 3 Task 29, ISO-59004 — FDIS PDF with a watermark).** (1) `AND` / `OR` in the FORMULA grammar combine COMPARISONS, not CALLS: `parseNumeric("if(contains(p,'a') AND contains(p,'b'), 1, 0)")` ⇒ `{"ok":false,"message":"Ausdruck erwartet."}` while a single `contains(...)` inside `if()` parses and `if(x == 1 AND y == 2, 1, 0)` parses — so every "all N checklist items ticked" code is blocked by the GRAMMAR before it is blocked by the missing `carriers`; write the nested-`if` form if you need a parseable record (`reduceRight` over the tokens), export BOTH and pin both behaviours (iso59004-F-1 / I-2). (2) The GATE condition grammar has no `contains` at all — `parseCondition("contains(principles, 'x')")` ⇒ null, only `principles IS NOT NULL` parses; a checklist therefore cannot condition N requirements that each name ONE of its items, and a shared presence check across N distinct CRs is an INVENTED enforcement — leave them empty-conditioned with the probe as the reason (iso59004-I-3 / G-4). (3) `count_rows(reg, col != '')` PARSES but returns `manual_required: Fehlende Eingabe für count_rows(): <col>`; `count_rows(reg, col IS NOT NULL)` computes AND already treats a stored EMPTY STRING as absent (probed side by side) — use `IS NOT NULL` for "field filled in" and pin both so a silent engine change fails the test instead of moving a count. (4) A WATERMARKED PDF ("Projet de Norme Marocaine" here): `pdftotext` emits the overlay's glyph runs as extra LINES between body lines, and occasionally glues a fragment to the front of a body word (`deGenerate`, `ojnongovernmental`). Keep the RAW span in every `verbatim_quote` (the verifier's whitespace collapse still matches), and add a build-time normaliser that drops only whole watermark-ONLY lines by an EXPLICIT token list (the letters of the watermark phrase) for the containment guard and for quotes rendered as prose — never clean a stored quote, and where a fragment is glued INSIDE a word pick a different, clean span and say so. (5) `-layout` on a TWO-COLUMN prose table interleaves the two column streams (a description printed on the NEXT row's label line; a label printed inside the previous row's description): the row↔cell association is then not recoverable without reconstruction, so the table is NOT seeded — and the claim is made re-executable by ASSERTING the interleaving against the committed span in the seed test (`expect(span).toContain('Re-mine deGenerate useful energy…')`), so "unseedable" is evidence, not an opinion. What IS still settled is the key column when each key stands alone on its own line: assert its printed ORDER and pin that it equals the prod enum, so the ENUM is PDF-verified even when the TABLE is not. (6) Free cross-check of the form-feed page mapping: when prod's own `verification_quote` cells already record "printed p.N … = PDF p.M" (this standard's do, in nine places), compute the mapping from the extraction and compare — nine independent agreements cost nothing and turn the page numbers from a derivation into a corroborated fact. Watch the off-by-one: a line that BEGINS with the form feed belongs to the NEXT page (`page(i) = 1 + formfeeds before line i + leading formfeeds OF line i`). (7) A single-select "current stage / current phase" field is NOT a visibility driver when the standard prints that the stages may run in parallel ("NOTE The sequence of stages can differ and can also occur at the same time or in parallel") — hiding the other stages' fields would make every gate on them unsatisfiable (the F-4 class). Withhold on the printed sentence FIRST and cite the gate-guard refusals only as the independent second reason; record the rules the guard would ACCEPT too, so the withholding reads as a judgment and not as a guard artefact. (8) `registerColumn.type` has no MULTI-select (text|number|boolean|enum|date|lookup_key|lookup_value|derived|grid): a printed "assessed against the following dimensions:" + N bullets becomes a `text` column with the N as its `datalist`, and any gate over it is only a presence check — record the missing column type as an I-block (iso59004-I-1). (9) When a brief names an enum token prod does not have (`recover` for `recover_energy`; a "more than one level" option that does not exist), do NOT substitute the nearest token — record the failing grep WITH its exit code, and either drop the rule (fail-safe visible) or create a source-grounded boolean driver beside the enum; adding the option would be a D-1 violation on a non-null prod list.
