...
/** Formula (A.8) (L2277–L2281): PECONRE(X) = (EIRENE(X) − EORENE(X)) / (EITE(X) − EOTE(X)) ⋅ 100 — the legend reads "in %" (L2284); the block prints "⋅1000" (iso59020-J-9). */
...
export const FIELD_CONFIGS: FieldConfigEntry[] = [
  ...
  WS04({
    ...
    ui_config: {
      ...
      note: `${Q.L1068_1071} Die 13 Zeilen von Tabelle 3 sind von Hand anzulegen (kein Vorbefüllen aus der Tabelle — iso59020-F-1); eine doppelte oder fehlende verbindliche Zeile setzt mandatory_core_indicators_included_code auf 0 (je Token genau einmal — iso59020-J-2 / F-2). Die Einzelfelder selected_core_indicator / indicator_category / indicator_not_applicable_justified / mandatory_core_indicators_included bleiben (iso59020-D-1 … D-4).`,
    },
  ...
  derivedOut(WS04, 'D', 'mandatory_core_indicators_included_code', 'Verbindliche Kernindikatoren vollständig einbezogen (1 = ja, 0 = nein)', null, '§7.3.1', 'ISO-59020-04-D5', '1, wenn jeder der sechs verbindlichen Indikatoren von Tabelle 3 (A.2.2, A.2.3, A.2.4, A.3.3, A.3.4, A.3.5) genau einmal als ausgewählt oder begründet nicht anwendbar erfasst ist — je Token count_rows(indicators, indicator == Token AND ok == 1) == 1 (iso59020-J-2); Zwilling des Eingabefelds mandatory_core_indicators_included (iso59020-D-4, Gate CR-009 → iso59020-G-4).', Q.L1068_1071),
  ...
  WS05({
    ...
    ui_config: {
      ...
      footer: ['inflows_count', 'm_ti_total', 'm_linear_total', 'inflows_unbalanced'],
      note: `${Q.L1892_1894} ${Q.L1850_1854} Eine Aggregation über die Zuflüsse (Σ/Σ oder Mittelwert — Anhang G.2 EXAMPLE b / c) ist bis zur Ratifizierung von iso59020-J-1 zurückgehalten. Die Einzelfelder mTI_X / mREUI_X / mRECI_X / mRENI_X / pct_linear_inflow bleiben (iso59020-D-6 … D-10); Gleichungen A.1 – A.3 und CR-011 … CR-014 → iso59020-R-1 / G-3.`,
    },
    ...
    create: { section_code: 'F', label_de: 'Ressourcenzuflüsse X (je Zufluss: mTI, mREUI, mRECI, mRENI; Anteile nach A.1 – A.3, linearer Rest, 100 %-Bilanz)', data_type: 'json', unit: null, clause_reference: 'A.2.1 – A.2.4, Formula (A.1) – (A.3)',
      description: 'Plan 3: Zeilen je Ressourcenzufluss X („Different types of inflow resources … should be measured and recorded separately“) mit den Formeln (A.1) – (A.3) je Zeile und dem linearen Rest (A.2.1: „subtracting the circular inflows from 100 %“); Anzahl / Σ Massen / Bilanzverstöße → ISO-59020-05-D1 … D7; die massengewichteten Anteile (Σ/Σ) sind bis zur Ratifizierung von iso59020-J-1 zurückgehalten. Die Einzelfelder bleiben (iso59020-D-6 … D-10).' },
  }),
  ...
  derivedOut(WS05, 'F', 'inflows_unbalanced', 'Zuflüsse mit verletzter 100 %-Bilanz', null, 'A.2.1', 'ISO-59020-05-D7', 'Zeilen mit balanced = 0 (zirkuläre Massen > mTI); CR-014 → iso59020-G-3.', Q.L1850_1854),
  ...
  WS06({
    ...
    ui_config: {
      ...
      footer: ['outflows_count', 'm_to_total', 'm_linear_out_total', 'outflows_unbalanced', 'outflows_untraceable'],
  ...
  derivedOut(WS06, 'G', 'outflows_unbalanced', 'Abflüsse mit verletzter 100 %-Bilanz', null, 'A.3.1', 'ISO-59020-06-D7', 'Zeilen mit balanced = 0 (zirkuläre Massen > mTO); CR-019 → iso59020-G-3.', Q.L2022_2026),
  derivedOut(WS06, 'G', 'outflows_untraceable', 'Abflüsse ohne rückverfolgbare Recyclingdaten (PRECO = 0 %)', null, 'A.3.4', 'ISO-59020-06-D8', 'Zeilen mit traceable_recycling = false („0 % should be recorded“).', Q.L2175),
  ...
  WS07({
    ...
    ui_config: {
      ...
      footer: ['energy_flows_count', 'ei_te_total', 'eo_te_total', 'energy_unit_mismatch'],
      note: `${Q.L2271_2272} Die Zeile gilt als abweichend, solange die gemeinsame Einheit dieses Arbeitsblatts (energy_unit_common) nicht gewählt ist. Formula (A.8) druckt „⋅1000“, die Legende „in %“ — die Zeile rechnet · 100 (iso59020-J-9). Die Einzelfelder EIRENE_X / EORENE_X / EITE_X / EOTE_X bleiben (iso59020-D-18 … D-21); Gleichung A.8 und CR-020 → iso59020-R-1 / G-1.`,
    },
    ...
    create: { section_code: 'C', label_de: 'Energieflüsse X (je Fluss: Einheit, EIRENE, EORENE, EITE, EOTE; PECONRE nach A.8)', data_type: 'json', unit: null, clause_reference: 'A.4.2, Formula (A.8)',
      description: 'Plan 3: Zeilen je Energiefluss X mit Formula (A.8) je Zeile und der Einheitsprüfung gegen energy_unit_common („A common suitable measurement unit (e.g. MJ, kWh) shall be selected“); Anzahl / Σ Energien / Einheitsabweichungen → ISO-59020-07-D1 … D6 (die Σ sind nur bei energy_unit_mismatch = 0 sinnvoll, §A.4.2); der Anteil über die Summen ist bis zur Ratifizierung von iso59020-J-1 zurückgehalten. Die Einzelfelder bleiben (iso59020-D-18 … D-21).' },
  }),
  ...
  derivedOut(WS07, 'C', 'energy_unit_mismatch', 'Energieflüsse mit abweichender Einheit', null, 'A.4.2', 'ISO-59020-07-D6', 'Zeilen mit unit_ok = 0 (Einheit ≠ energy_unit_common oder keine gemeinsame Einheit gewählt); Gates STAGED (iso59020-G-5 block / G-6 warn).', Q.L2271_2272),
