 ...
 * ISO-59020-05-D2 / D6 / D7 sums and counts through the real
 * `evaluateFormula` (the Σ/Σ aggregates are withheld — J-1); typing a mass moves the derived cells and the sums; a new
...
describe('inflows through the generic RegisterEditor (Plan 3 Task 21)', () => {
  it('two inflow rows show %REUI / %RECI / PRENI per row (20 / 50 / 0 and 0 / 0 / 75), the linear share (30 / 25 %) and the balance badge; Σ mTI = 1400 kg, Σ linear = 400 kg, unbalanced 0', () => {
    ...
    expect(evalOut('ISO-59020-05-D6', 'inflows', INFLOWS, storedRows(INFLOWS_ID))).toMatchObject({ kind: 'computed', value: 400 });
    expect(evalOut('ISO-59020-05-D7', 'inflows', INFLOWS, storedRows(INFLOWS_ID))).toMatchObject({ kind: 'computed', value: 0 });
  });

  it('typing mREUI = 700 on the steel row flips its balance badge (700 + 500 > 1000) and ISO-59020-05-D7 counts it; a new row without its label is incomplete and never enters the sums', async () => {
    ...
    expect(evalOut('ISO-59020-05-D7', 'inflows', INFLOWS, storedRows(INFLOWS_ID))).toMatchObject({ kind: 'computed', value: 1 });
...
describe('outflows through the generic RegisterEditor (Plan 3 Task 21)', () => {
  it('a row without traceable recycling data hides the mRECO input and reads PRECO 0 % (A.3.4), stays complete; the traceable row shows 30 %; ISO-59020-06-D8 counts the untraceable row, -D4 sums only the traceable mRECO', () => {
    ...
    expect(evalOut('ISO-59020-06-D8', 'outflows', OUTFLOWS, storedRows(OUTFLOWS_ID))).toMatchObject({ kind: 'computed', value: 1 });
    expect(evalOut('ISO-59020-06-D4', 'outflows', OUTFLOWS, storedRows(OUTFLOWS_ID))).toMatchObject({ kind: 'computed', value: 300 });
  });
});
