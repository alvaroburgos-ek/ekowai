...
describe('LookupFillField — I-1: fill honours field.dataType (text / enum), never writes a number into a non-number field', () => {
  ...
  it('Plan 3 Task 0 (2b review F-4): a STORED number over a non-numeric cell (tableScalar === null) is not "abweichend" — nothing to deviate from', () => {
    registerTables([stringTable()]);
    const setField = vi.fn();
  ...
  it('Plan 3 Task 0 (2b review F-4), enum twin: a STORED option over a cell outside the options ⇒ mismatch badge, still no "abweichend"', () => {
    registerTables([stringTable()]);
    const setField = vi.fn();
